...
Arduino をまったく触ったことがない方でも進められるよう、順番どおりに書いています。
...
[2] 必要なライブラリを入れる
        ↓
[3] プログラムを用意する   ← 方法A（自分で編集）／方法B（編集済みを入手）
        ↓
[4] Zeus Carに書き込む
        ↓
[5] 動作確認
...
方法Bを選ぶ場合は、[3] でパラメータ編集が不要になります（詳細は後述）。
...
## ステップ4：プログラムを用意する（方法A / 方法B）

プログラム（コード）を用意する方法は 2 通りあります。**どちらか一方**を選んでください。

### ▼ 方法A：公式コードを入手して、自分でパラメータを編集する
...
5. **→ このあと「第2部：パラメータの変更」に進み、数値を書き換えてください。**

> 📖 **公式の画像付き手順**：[コードのダウンロード](https://docs.sunfounder.com/projects/zeus-car/ja/latest/get_started/download_code.html)

### ▼ 方法B：調整済みコードを SharePoint（SPO）から入手して、そのまま書き込む

すでに速度が調整されたコードを SharePoint Online（SPO）の共有リンクから受け取る場合はこちら。**パラメータ編集は不要**で、そのまま書き込みに進めます。
...
4. **→ 第2部は飛ばして、第3部：書き込み手順に進んでください。**

---

# 第2部　パラメータの変更（方法Aの人のみ）

方法B（SharePoint で入手）を選んだ人は、この第2部を飛ばして第3部に進んでください。

数値を大きくすると速く、小さくすると遅くなります。値の範囲は原則 **0〜100** です。

> 初回だけ、旋回を独立設定できるようにするための下ごしらえ（2-3）が必要です。一度対応すれば、以降は `Zeus_Car.ino` の数字を変えるだけで 4 つの速度をすべて調整できます。
...
App はジョイスティックの倒し量で速度が変わります。その**最大値の上限**を決めます。
...
旋回は App もリモコンも、内部の「方位を保つ計算（PID）」を共通で通ります。そのままでは App とリモコンの旋回を別々にできないため、**旋回の上限を操作方法ごとに切り替えられるよう、最初に一度だけプログラムを少し改造**します。

編集するファイルは 3 つです。落ち着いて上から順に置き換えてください。
...
以降は、`IR_ROTATE_LIMIT` と `APP_ROTATE_LIMIT` の数値を変えるだけで、App とリモコンの旋回速度を別々に調整できます。

## 2-3. パラメータ早見表（方法Aの調整対象）

初回の改造が済めば、**日常の調整はすべて `Zeus_Car.ino` の 4 つの数字**だけです。
...
> **値の目安**：移動速度を 40 より小さくすると発進しづらくなることがあります。小さめにしたいときは実機で確認しながら少しずつ下げてください。

編集が終わったら、**すべてのファイルを保存**して第3部へ進みます。

---

# 第3部　Zeus Car への書き込み

ここは方法A・方法B 共通です。**順番が重要**なので、そのとおりに行ってください。
...
1. Arduino IDE 上部のボード選択欄で **「Arduino Uno」** を選びます。
2. 接続先（ポート）の一覧から Zeus Car のポートを選びます。Windows では `COM3` のような表示、Mac では `/dev/cu.usbserial…` のような表示になります。どれか分からない場合は、USB を抜き差しして新しく現れる項目が Zeus Car です。

## 3-4. 書き込む

1. Arduino IDE 左上の **「→（書き込み / Upload）」ボタン**を押します。
2. 自動的にコンパイル（変換）→ 書き込みが行われます。下部に **「Done uploading」** と出れば成功です。
...
この上限値こそが「旋回の勢い」を決めています。第2部の改造では、この固定の `±100` を **引数 `rotLimit` に置き換え**、App とリモコンで別々の上限を渡せるようにしました。これにより、共通だった旋回速度を操作方法ごとに独立させています。
...
第2部で編集した `REGION_K` や `REGION_Q` は、SunFounder Controller アプリ上の各ウィジェット（操作パネル）に対応しています。公式の [APP Control Plus](https://docs.sunfounder.com/projects/zeus-car/ja/latest/get_started/ar_app_control_plus.html) の配置に沿って、今回の調整に関係するものを整理すると次のとおりです。
...
つまり `REGION_K` に上限をかける第2部の編集は「移動ジョイスティック」の最大速度を、`REGION_Q` から入る旋回に `APP_ROTATE_LIMIT` を効かせる編集は「方向ジョイスティック」の旋回速度を、それぞれ制御していることになります。
