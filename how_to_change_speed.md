...
## H. 速度と登坂力（トルク）のトレードオフ

**移動速度を下げると、登坂力（坂を登る力）も一緒に下がります。** これは今回の速度調整が、モーターへの平均電圧（PWM のデューティ比）を下げる方式のためです。DC モーターの最大トルクは印加電圧にほぼ比例するので、`power` を下げて実効電圧を落とすと、最高速度と最大トルクが同じノブで連動して下がります。つまり速度を落とすと、坂で必要な「押し出す力の余力」も削られます。

坂での挙動で言うと、平地では軽負荷でモーターは速く回りますが、坂では負荷で回転が落ち、電流（＝トルク）が増えて自然に踏ん張ります。ところが PWM に上限をかけていると、この「電流を増やしてトルクを出す」動作の天井も低くなるため、本来なら登れた勾配でストール（止まる）しやすくなります。

**パラメータ別の影響**

| パラメータ | 登坂力への影響 |
|---|---|
| `IR_REMOTE_POWER` / `APP_REMOTE_POWER`（移動速度） | **直接下がる**（速度と登坂力が連動） |
| `IR_ROTATE_LIMIT` / `APP_ROTATE_LIMIT`（旋回速度） | 旋回成分だけを制限するので、**直進の登坂力には影響しない** |

つまり、速度を下げつつ登坂力をなるべく保ちたいなら、**移動速度を下げすぎず**（下限を確保）、「体感的なマイルドさ」は**旋回側の制限で稼ぐ**のが有効です。

**補足**
- この firmware は `carMove()` 内で入力 `power` を約 0.35 倍にスケールしてからモーターに渡しています。元々低めのデューティで走っているため、そこからさらに下げるとトルク余裕を早く食いつぶします。
- 起動時の `MOTOR_START_POWER` キック（約 200µs）は静止摩擦を切って動き出すためのもので、坂を登り続ける持続トルクには寄与しません。
- 速度と力をきれいに分離する（速度だけ落として力は保つ）には本来ギア比やタイヤ径の変更が必要です。Zeus Car は車輪エンコーダを持たないため、ソフトでのクローズドループ速度制御もできません。
- **落としどころ**：移動速度を下げすぎない／マイルドさは旋回制限で稼ぐ／実際に使う勾配・段差で実機確認する。展示会場に段差やスロープがあるなら、その場で登れる最低 `power` を確認して設定値を決めるのが確実です。
